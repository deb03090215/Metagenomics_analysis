...
Filters a BLAST TSV report for viral contigs and applies a length threshold
based on the 95th percentile (with a minimum of 300 bp and a maximum cap of 900 bp).
The output CSV filename is set to 'BLAST_parsing.csv'.
...
def main():
    parser = argparse.ArgumentParser(
        description='Filter BLAST report for Viruses and apply length threshold.'
    )
    parser.add_argument(
        '--input', '-i', required=True,
        help='Path to the BLAST TSV file'
    )
    parser.add_argument(
    ...
    # 1. Read BLAST TSV file into DataFrame
    df = pd.read_csv(blast_file, sep='\t', low_memory=False)
    ...
    virus_df.to_csv('BLAST_virus.csv', index=False)

    # 6. Check for 'length' column and calculate percentile threshold
    if 'length' not in virus_df.columns:
        print("Error: 'length' column not found in BLAST_virus.csv.")
        sys.exit(1)
